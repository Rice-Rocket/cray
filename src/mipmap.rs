use std::{ops::{Add, Div, Mul}, path::PathBuf, sync::Arc};
...
impl MIPMap {
    ...
    pub fn filter<T: TexelType>(&self, st: Point2f, mut dst0: Vec2f, mut dst1: Vec2f) -> T {
...
pub trait TexelType: Add<Self, Output = Self> + Mul<Float, Output = Self> + Div<Float, Output = Self> + Sized + Default {
    ...
    fn ewa(mipmap: &MIPMap, level: usize, mut st: Point2f, mut dst0: Vec2f, mut dst1: Vec2f) -> Self {
...
impl TexelType for Float {
    ...
    fn bilerp(mipmap: &MIPMap, level: usize, st: Point2f) -> Self {
        match mipmap.pyramid[level].n_channels() {
            1 => mipmap.pyramid[level].bilinear_channel_wrapped(st, 0, mipmap.wrap_mode.into()),
            3 => mipmap.pyramid[level].bilinear(st, mipmap.wrap_mode.into()).average(),
            4 => mipmap.pyramid[level].bilinear_channel_wrapped(st, 3, mipmap.wrap_mode.into()),
            _ => panic!("unexpected number of image channels"),
        }
    }
}
...
impl TexelType for Rgb {
    ...
    fn bilerp(mipmap: &MIPMap, level: usize, st: Point2f) -> Self {
        ...
        if nc == 3 || nc == 4 {
            Rgb::new(
                mipmap.pyramid[level].bilinear_channel_wrapped(st, 0, mipmap.wrap_mode.into()),
                mipmap.pyramid[level].bilinear_channel_wrapped(st, 1, mipmap.wrap_mode.into()),
                mipmap.pyramid[level].bilinear_channel_wrapped(st, 2, mipmap.wrap_mode.into()),
            )
        } else {
            debug_assert!(nc == 1);
            let v = mipmap.pyramid[level].bilinear_channel_wrapped(st, 0, mipmap.wrap_mode.into());
            Rgb::new(v, v, v)
        }
    }
}
