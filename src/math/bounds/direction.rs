...
impl DirectionCone {
    ...
    pub fn new(w: Unit<Vec3>, cos_theta: Scalar) -> Self {
        Self { w, cos_theta }
    }

    pub fn from_direction(w: Unit<Vec3>) -> Self {
        Self { w, cos_theta: 1.0 }
    }

    pub fn bound_subtended_directions(b: &Bounds3, p_unit: Unit<Vec3>) -> Self {
        let p = p_unit.into_inner();
        let (center, radius) = b.bounding_sphere();
        if (p - center).magnitude_squared() < math::sqr(radius) {
            return Self::entire_sphere();
        }
        let w = Unit::new_normalize(p - center);
    ...
    pub fn union(&self, rhs: &Self) -> Self {
        ...
        Self { w: Unit::new_unchecked(w.xyz()), cos_theta: theta_o.cos() }
    }
    ...
    pub fn inside(&self, w: &Unit<Vec3>) -> bool {
        !self.is_empty() && self.w.dot(w) >= self.cos_theta
    }

    pub fn entire_sphere() -> Self {
        Self { w: Unit::new_unchecked(Vec3::new(0.0, 0.0, 1.0)), cos_theta: -1.0 }
    }
}
